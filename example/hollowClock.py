...
def drawClock(mc, clockCentre, radius, time):
    ...
    #draw hour hand
    drawHourHand(mc, clockCentre, time.hour, time.minute,14)
    
    #draw minute hand
    drawMinuteHand(mc, clockCentre, time.minute, 14)

    #draw second hand
    drawSecondHand(mc, clockCentre, time.second, 22)
...
def updateTime(mc, clockCentre, lastTime, time):
    ...
    if (lastTime.minute != time.minute):
        #clear hour hand
        drawHourHand(mc, clockCentre, lastTime.hour, lastTime.minute, 0)
        #new hour hand
        drawHourHand(mc, clockCentre, time.hour, time.minute, 14)
        
        #clear hand
        drawMinuteHand(mc, clockCentre, lastTime.minute, 0)
        #new hand
        drawMinuteHand(mc, clockCentre, time.minute, 14)
    ...
    if (lastTime.second != time.second):
        #clear hand
        drawSecondHand(mc, clockCentre, lastTime.second, 0)
        #new hand
        drawSecondHand(mc, clockCentre, time.second, 22)
